...
async function cargarBodegas() {
  ...
  try {
    ...
    todasLasBodegas = Array.isArray(bodegas) ? bodegas : (bodegas?.content || []);
...
document.getElementById('bodega-form')?.addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    encargado: form.elements.encargado.value ? { id: Number(form.elements.encargado.value) } : null,
  };
...
async function cargarProductos() {
  ...
  try {
    ...
    todosLosProductos = Array.isArray(raw) ? raw : (raw?.content || []);
...
async function cargarMovimientos() {
  ...
  try {
    const raw = await apiFetch('/api/movimientos');
    todosLosMovimientos = Array.isArray(raw) ? raw : (raw?.content || []);
...
async function cargarAuditorias() {
  ...
  try {
    const raw = await apiFetch('/api/auditorias');
    todasLasAuditorias = Array.isArray(raw) ? raw : (raw?.content || []);
